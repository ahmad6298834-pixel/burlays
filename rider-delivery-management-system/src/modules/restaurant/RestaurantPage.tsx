...
export default function RestaurantSettingsPage() {
  ...
  return (
    <div>
      ...
      {!loading && tab === "info" && <InfoTab info={info} send={send} />}
      {!loading && tab === "categories" && <CategoriesTab categories={categories} send={send} />}
      {!loading && tab === "menu" && <MenuItemsTab items={items} categories={categories} send={send} />}
      {!loading && tab === "deals" && <DealsTab deals={dealList} items={items} send={send} />}
      {!loading && tab === "extras" && <ExtrasTab extras={extras} send={send} />}
      {!loading && tab === "payments" && <PaymentsTab payments={payments} send={send} />}
    </div>
  );
}
...
function InfoTab({ info, send }: { info: RestaurantSettings | null; send: Send }) {
...
function CategoriesTab({ categories, send }: { categories: MenuCategory[]; send: Send }) {
  ...
  return (
    <div className="space-y-4">
          ...
          {visible.map((c) => (
            <Card key={c.id} className="p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <Button variant="secondary" className="w-full" onClick={() => openEdit(c)}>
                  Edit
                </Button>
                <Button
      ...
      {open && (
        <Modal title={editing ? "Edit Category" : "Add Category"} onClose={() => setOpen(false)} onSave={save}>
        ...
        <div className="space-y-3">
          {visible.map((item) => (
            <Card key={item.id} className="p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <Button variant="secondary" className="w-full" onClick={() => openEdit(item)}>
                  Edit
                </Button>
                <Button
...
function DealsTab({ deals, items, send }: { deals: DealWithItems[]; items: MenuItemWithSizes[]; send: Send }) {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [open, setOpen] = useState(false);
        ...
        <div className="space-y-3">
          {visible.map((deal) => (
            <Card key={deal.id} className="p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <Button variant="secondary" className="w-full" onClick={() => openEdit(deal)}>
                  Edit
                </Button>
                <Button
...
function ExtrasTab({ extras, send }: { extras: MenuExtra[]; send: Send }) {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [open, setOpen] = useState(false);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={openAdd}>+ Add Extra</Button>
      </div>
        ...
        <div className="space-y-3">
          {visible.map((x) => (
            <Card key={x.id} className="p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <Button variant="secondary" className="w-full" onClick={() => openEdit(x)}>
                  Edit
                </Button>
                <Button
      ...
      {open && (
        <Modal title={editing ? "Edit Extra" : "Add Extra"} onClose={() => setOpen(false)} onSave={save}>
          <div className="space-y-3">
            <div>
...
function PaymentsTab({ payments, send }: { payments: PaymentMethod[]; send: Send }) {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState("all");
  const [open, setOpen] = useState(false);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button onClick={openAdd}>+ Add Payment Method</Button>
      </div>
        ...
        <div className="space-y-3">
          {visible.map((m) => (
            <Card key={m.id} className="p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <Button variant="secondary" className="w-full" onClick={() => openEdit(m)}>
                  Edit
                </Button>
                <Button
      ...
      {open && (
        <Modal title={editing ? "Edit Payment Method" : "Add Payment Method"} onClose={() => setOpen(false)} onSave={save}>
          <Label>Name</Label>
