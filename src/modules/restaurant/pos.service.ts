...
export type CreateOrderInput = {
  ...
  /** Idempotency key so a duplicate submit cannot create a second order. */
...
export async function createRestaurantOrder(input: CreateOrderInput) {
  // If this exact submission was already processed, return the original order
  // instead of creating a duplicate.
  if (input.clientRequestId) {
    const [dupe] = await db
      .select()
      .from(restaurantOrders)
      .where(eq(restaurantOrders.clientRequestId, input.clientRequestId))
      .limit(1);
    if (dupe) return { order: dupe, duplicate: true as const };
  }
  ...
  let created: typeof restaurantOrders.$inferSelect | undefined;
  try {
    [created] = await db.insert(restaurantOrders).values({ ...values, orderNumber }).returning();
  } catch (err) {
    // A concurrent duplicate won the race on the unique key — return that order.
    if (input.clientRequestId) {
      const [dupe] = await db
        .select()
        .from(restaurantOrders)
...
export async function getRestaurantOrder(id: number): Promise<RestaurantOrderFull | null> {
  const [order] = await db
    .select()
    .from(restaurantOrders)
    .where(and(eq(restaurantOrders.id, id), isNull(restaurantOrders.deletedAt)))
    .limit(1);
...
export async function listRestaurantOrders(limit = 50) {
  // Soft-deleted orders are hidden from the list and from all reporting.
  const rows = await db
    .select()
    .from(restaurantOrders)
    .where(isNull(restaurantOrders.deletedAt))
