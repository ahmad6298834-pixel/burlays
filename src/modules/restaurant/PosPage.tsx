...
import { ORDER_TYPES, ORDER_TYPE_LABELS, RESTAURANT_NAME } from "@/types";
import type {
  DealWithItems,
  Location,
  MenuCategory,
...
export default function PosPage() {
  ...
  // Stable per-cart key: retries/double-clicks reuse it so the server de-dupes.
  const requestId = useRef<string>("");

  useEffect(() => {
    fetch("/api/restaurant/categories").then((r) => r.json()).then(setCategories);
    ...
    // Payment methods and restaurant info come from Settings, never hard-coded.
    fetch("/api/restaurant/payment-methods")
      .then((r) => r.json())
      .then((d: PaymentMethod[]) => {
        const active = d.filter((m) => m.active);
  ...
  const refreshOrderNumber = () => {
    fetch("/api/restaurant/orders?preview=number")
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    lookupTimer.current = setTimeout(async () => {
      try {
        ...
        const c: CustomerLookup | null = data.customer;
        if (c) {
          setCustomerName(c.name);
          if (c.defaultLocationId && orderType === "delivery") applyLocation(String(c.defaultLocationId));
  ...
  /**
   * Selecting a location loads the location's configured charge. That figure is the
   * RIDER's internal earning; we also pre-fill the customer charge with it as a
   * convenience, but the admin can override the customer figure independently.
   */
  ...
  /* ---------------------------------- cart ---------------------------------- */
  /**
   * Opens the chooser so the operator can pick a size (required for pizza-style
   * items) and any extras. Items with neither are added straight to the cart.
   */
  ...
  const addToCart = (item: MenuItemWithSizes, sizeId: number | null, extraIds: number[]) => {
    ...
    // Extras are part of the line identity so the same item with different
    // add-ons stays on its own line.
  ...
  const custCharge = orderType === "delivery" ? Number(customerDeliveryCharge) || 0 : 0;
  ...
  const save = async () => {
    ...
    if (saving || savedId) return; // guard against double submission
    ...
    if (!res.ok) {
      ...
      setError(d.error ?? "Could not save order");
      return;
    }
    const order = await res.json();
    setSavedId(order.id);
    setOrderNumber(order.orderNumber);
    // Go straight to the combined print flow; it returns the user to
    // Restaurant Orders once the print dialog closes.
    router.push(`/restaurant/orders/${order.id}/print?autoprint=1`);
  };
  ...
  const showDelivery = orderType === "delivery";

  return (
    <div>
      <PageHeader
        title={`${settings?.name ?? RESTAURANT_NAME} — New Order (POS)`}
        subtitle="Take dine-in, takeaway and delivery orders."
      />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4">
                ...
                <div>
                  ...
                  {lookupStatus === "found" && (
                    <p className="mt-1 text-xs font-medium text-emerald-600">
                      ✅ Existing customer — details auto-filled (editable).
                    </p>
                  )}
                  {lookupStatus === "new" && (
                    <p className="mt-1 text-xs text-slate-400">New customer — will be saved.</p>
                  )}
                </div>
                <div>
          ...
          <Card>
            ...
            <Button className="mt-4 w-full" onClick={save} disabled={saving || cart.length === 0 || Boolean(savedId)}>
              {saving ? "Saving..." : savedId ? "Saved — printing..." : "💾 Save Order"}
            </Button>
          </Card>
        </div>
      </div>
